...
bool dfs(int v, int p, vvi& adj_list, vi& arv, vi& dep, vi &par, int& timer)
{
    arv[v] = timer++;
    par[v] = p;
    ...
    for(int adj : adj_list[v])
    {
        ...
        {
            if(dfs(adj, v, adj_list, arv, dep, par, timer))
                return true;
            continue;
        }
...
signed main()
{
    ...
    forn(i, 1, n + 1)
    {
        if(arv[i] != 0)
            continue;
        if(dfs(i, -1, adj_list, arv, dep, par, timer))
        {
           impossible = false;
           break;
        }
